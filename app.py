...
with st.sidebar:
    ...
    # ── PEDACITO INTEGRADO: ESPECIALIDADES ──
        ...
        if st.session_state.patient_id:
            ...
            if len(visitas) > 1:
                with st.expander(f"📂 {len(visitas)-1} visita(s) previa(s)", expanded=False):
                    for v in visitas[1:]:
                        st.markdown(f"<div style='font-size:0.7rem
¡Tranquilo, hermano! Es normal, con tanto código uno se marea. Te lo devuelvo completo ya con el pedacito insertado en el lugar exacto. Cópialo todo y reemplázalo en tu proyecto:
