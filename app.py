"""
app.py – Hospital Rísquez · OphthalmAI v3.2
Sistema de Apoyo Diagnóstico Oftalmológico
Especialidad: Úlceras Corneales y Uveítis
"""
...
# ──────────────────────────────────────────────────────────────────────────────
# OJO SVG — FUNCIÓN ÚNICA (mismo diseño, tamaños distintos)
# Inspirado en el estilo del proyecto Arena · hiperrealista
# ──────────────────────────────────────────────────────────────────────────────
def eye_svg(width="120px", uid="a", pulse_dur="3s", radar_dur="2.6s"):
    """Retorna el SVG del ojo como string HTML. uid evita conflictos de IDs."""
    return f"""<svg viewBox="0 0 220 120" xmlns="http://www.w3.org/2000/svg"
  style="width:{width};height:auto;display:block;margin:0 auto;
         filter:drop-shadow(0 0 12px rgba(0,229,216,0.6))
                drop-shadow(0 0 32px rgba(0,229,216,0.25));">
  <defs>
    <!-- Esclerótica: blanco azulado natural con degradado central -->
    ...
    <!-- Iris: desde negro profundo al teal brillante -->
  ...
  <ellipse cx="110" cy="60" rx="108" ry="58"
    fill="none" stroke="#00e5d8" stroke-width="0.5" opacity="0.1"
    filter="url(#eg{uid})"/>
  <ellipse cx="110" cy="60" rx="112" ry="62"
    fill="none" stroke="#00cfff" stroke-width="0.3" opacity="0.07"
    filter="url(#eg{uid})"/>

  <!-- ── Esclerótica ── -->
  <path d="M6,60 Q110,4 214,60 Q110,116 6,60 Z"
    fill="url(#sc{uid})" clip-path="url(#ey{uid})"/>

  <!-- ── Venas oculares sutiles ── -->
  <g clip-path="url(#ey{uid})" opacity="0.13">
    ...
    <path d="M22,57 Q44,52 64,56" fill="none" stroke="#c0392b" stroke-width="0.4"/>
  ...
  <!-- ── Iris con gradiente profundo ── -->
  <circle cx="110" cy="60" r="36"
    fill="url(#ir{uid})" filter="url(#ig{uid})"
    clip-path="url(#ey{uid})"/>
  ...
  <g clip-path="url(#ey{uid})" opacity="0.19">
    ...
    <line x1="78"  y1="40" x2="142" y2="80" stroke="#00cfff" stroke-width="0.5"/>
    <line x1="78"  y1="80" x2="142" y2="40" stroke="#00cfff" stroke-width="0.5"/>
    <line x1="68"  y1="47" x2="152" y2="73" stroke="#00e5d8" stroke-width="0.4"/>
    <line x1="68"  y1="73" x2="152" y2="47" stroke="#00e5d8" stroke-width="0.4"/>
    <!-- Anillos concéntricos del iris -->
    ...
    <circle cx="110" cy="60" r="36" fill="none" stroke="#00e5d8" stroke-width="1" opacity="0.4"/>
  </g>

  <!-- ── Pupila ── -->
  <circle cx="110" cy="60" r="14.5"
    fill="url(#pu{uid})" clip-path="url(#ey{uid})"/>

  <!-- ── Anillo de dilatación animado ── -->
  <circle cx="110" cy="60" r="14.5" fill="none"
    stroke="#00e5d8" stroke-width="1.2" opacity="0.8"
    clip-path="url(#ey{uid})">
  ...
  <!-- ── Reflejos corneales (glints de luz) ── -->
  <ellipse cx="96" cy="47" rx="5.2" ry="2.8" fill="white" opacity="0.8"
    transform="rotate(-28,96,47)" clip-path="url(#ey{uid})"/>
  <ellipse cx="103" cy="45" rx="1.8" ry="1" fill="white" opacity="0.5"
    clip-path="url(#ey{uid})"/>
  <circle  cx="124" cy="70" r="1.4" fill="white" opacity="0.22"
    clip-path="url(#ey{uid})"/>

  <!-- ── Contorno natural de párpados ── -->
  <path d="M6,60 Q110,4 214,60"
    fill="none" stroke="#003028" stroke-width="2.4" opacity="0.65"/>
  <path d="M6,60 Q110,116 214,60"
    fill="none" stroke="#003028" stroke-width="2.4" opacity="0.65"/>

  <!-- ── Borde teal con glow ── -->
  <path d="M6,60 Q110,4 214,60 Q110,116 6,60 Z"
    fill="none" stroke="#00e5d8" stroke-width="1.4" opacity="0.6"
    filter="url(#eg{uid})"/>

  <!-- ── Radar pulse: anillos desde iris hacia afuera ── -->
  <ellipse cx="110" cy="60" rx="36" ry="36" fill="none"
    stroke="#00e5d8" stroke-width="2" opacity="0" clip-path="url(#ey{uid})">
  ...
  <ellipse cx="110" cy="60" rx="36" ry="36" fill="none"
    stroke="#00cfff" stroke-width="1.3" opacity="0" clip-path="url(#ey{uid})">
    <animate attributeName="rx"      values="36;108" dur="{radar_dur}" begin="1.3s" repeatCount="indefinite"/>
    <animate attributeName="ry"      values="36;58"  dur="{radar_dur}" begin="1.3s" repeatCount="indefinite"/>
...
# ──────────────────────────────────────────────────────────────────────────────
# UTILIDAD: generar resumen (definida aquí para ser siempre disponible)
# ──────────────────────────────────────────────────────────────────────────────
def _generar_resumen(doctor: str, paciente: str, mensajes: list) -> str:
    ...
    lineas += [
        "=" * 62,
        "Reporte generado por OphthalmAI v3.2 · Hospital Rísquez",
...
.sidebar-logo::before{content:'';position:absolute;top:0;left:50%;transform:translateX(-50%);width:60%;height:1px;background:linear-gradient(90deg,transparent,var(--teal),transparent);}
...
[data-testid="stSidebar"] input,.stTextInput input{background:rgba(5,13,26,0.9)!important;color:var(--teal)!important;border:1px solid var(--glass-brd)!important;border-radius:8px!important;font-family:'Space Mono',monospace!important;font-size:0.82rem!important;}
[data-testid="stSidebar"] input:focus{border-color:var(--teal)!important;box-shadow:0 0 0 2px rgba(0,229,216,0.12)!important;}
input::placeholder,textarea::placeholder{color:#1e3550!important;}
.stSelectbox>div>div{background:rgba(5,13,26,0.9)!important;border:1px solid var(--glass-brd)!important;color:var(--teal)!important;border-radius:8px!important;}
...
/* Chip buttons */
div[data-testid="column"] .stButton>button{font-family:'DM Sans',sans-serif!important;font-size:0.72rem!important;font-weight:500!important;padding:6px 4px!important;letter-spacing:0.3px!important;text-transform:none!important;border-color:rgba(0,229,216,0.25)!important;color:var(--text-muted)!important;border-radius:20px!important;}
div[data-testid="column"] .stButton>button:hover{border-color:var(--teal)!important;color:var(--teal)!important;background:rgba(0,229,216,0.06)!important;}
...
/* Status strip */
.status-strip{display:flex;align-items:center;gap:16px;margin-bottom:12px;padding:8px 16px;background:var(--panel);border:1px solid var(--glass-brd);border-radius:8px;font-family:'Space Mono',monospace;font-size:0.62rem;color:var(--text-muted);flex-wrap:wrap;}
...
/* Chat */
[data-testid="stChatMessage"]{background:var(--panel)!important;border:1px solid var(--glass-brd)!important;border-radius:12px!important;padding:16px 20px!important;margin-bottom:10px!important;backdrop-filter:blur(12px);position:relative;}
...
/* Welcome */
.welcome-screen{text-align:center;padding:50px 40px;border:1px solid var(--glass-brd);border-radius:20px;margin-top:14px;background:linear-gradient(135deg,rgba(0,229,216,0.03) 0%,transparent 50%),var(--panel);position:relative;overflow:hidden;}
.welcome-screen::before{content:'';position:absolute;top:-50%;left:-50%;width:200%;height:200%;background:conic-gradient(transparent 0deg,rgba(0,229,216,0.02) 60deg,transparent 120deg);animation:rotate-bg 14s linear infinite;}
...
with st.sidebar:
    ...
    <div class="sidebar-logo">
        {eye_svg(width="130px", uid="sb", pulse_dur="3.5s", radar_dur="2.9s")}
    ...
    <div class="s-info">
        ...
        REF: AAO · ESCRS · SVO · Hospital Rísquez
        </span>
    </div>
    """, unsafe_allow_html=True)
...
<div class="hud-header">
    <div style="flex-shrink:0;">
        {eye_svg(width="68px", uid="hd", pulse_dur="3s", radar_dur="2.5s")}
    </div>
    <div class="hud-title">
    ...
    if not st.session_state.session_active:
        ...
        <div class="welcome-screen">
            <div style="position:relative;z-index:1;">
                {eye_svg(width="150px", uid="wc", pulse_dur="4s", radar_dur="3.2s")}
            </div>
        ...
        # ── Exportar resumen (BUG FIXED: generado dentro de try/except, solo cuando hay mensajes) ──
        if st.session_state.total_consultas > 0:
            try:
                ...
                fname = (
                    f"consulta_{st.session_state.patient_name.replace(' ','_')}"
                    f"_{time.strftime('%Y%m%d_%H%M')}.txt"
                )
                st.download_button(
                ...
                pass  # No crashear si algo falla en la generación
        ...
        if prompt:
            ...
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    respuesta_ia = f"⚠️ **Error en módulo CNN:** `{e}`\n\nVerifique que `modelo_vision.py` esté en el directorio del proyecto."
...
# ────────────────────────────────────────────────────────────────────────
    ...
    with st.form("form_reg", clear_on_submit=True):
        ...
        antec     = st.text_area("Antecedentes Oftalmológicos y Médicos",
            placeholder="Diabetes, HTA, cirugías previas, lentes de contacto, HSV previo...", height=75)
        c8, c9    = st.columns(2)
        ...
        doc_reg   = st.text_input("Doctor que Registra *", value=st.session_state.doctor_name, placeholder="Dr. García")
